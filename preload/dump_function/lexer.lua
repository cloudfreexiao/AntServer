...
local BYTE_ZH_MIN = 128
local BYTE_ZH_MAX = 244
...
for i=BYTE_ZH_MIN, BYTE_ZH_MAX do
   mask[i] = "zh"
end

local function is_zh(b)
   return mask[b]=="zh"
   -- return b >= BYTE_ZH_MIN and b <= BYTE_ZH_MAX
end
...
local keywords = {
   ["and"] = true, ["break"] = true, ["do"] = true, ["else"] = true, ["elseif"] = true, ["end"] = true, ["false"] = true, ["for"] = true, ["function"] = true, ["goto"] = true, ["if"] = true, ["in"] = true,
   ["local"] = true, ["nil"] = true, ["not"] = true, ["or"] = true, ["repeat"] = true, ["return"] = true, ["then"] = true, ["true"] = true, ["until"] = true, ["while"] = true,
}
...
local function lex_ident(state)
   ...
   while (b ~= nil) and (is_alpha(b) or to_dec(b) or is_zh(b)) do
      b = next_byte(state)
   end
...
for b=BYTE_ZH_MIN, BYTE_ZH_MAX do
   byte_handlers[b] = lex_ident
end
...
function lexer.syntax_error(location, end_column, msg)
   error(setmetatable({
      ...
      __tostring = function (t)
         local s = string.format("%s @line:%d column:%d end_column:%d", 
            t.msg, t.line, t.column, t.end_column)
         return s
      end}
      )
   )
