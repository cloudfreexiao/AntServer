...
namespace Skynet.DotNetClient.Gate.UDP
{
    public class ByteBuffer : ICloneable
    {
        ...
        private ByteBuffer(byte[] bytes)
        {
            ...
            Array.Copy(bytes, 0, _buf, 0, _buf.Length);
            this._capacity = _buf.Length;
            this.readIndex = 0;
        ...
        {
            // 如果没有足够的空间进行写入了
            if (WritableBytes >= minBytes) return;
            // 优先整理空间
            ...
            {
                // 空间不足时，重新分配内存
                FixSizeAndReset(_buf.Length, _buf.Length + minBytes);
            }
        }

        public void TrimReadedBytes()
        {
            Buffer.BlockCopy(_buf, readIndex, _buf, 0, writeIndex - readIndex);
        ...
        public void WriteBytes(byte[] bytes, int startIndex, int length)
        {
            ...
            Array.Copy(bytes, startIndex, _buf, writeIndex, length);
            writeIndex = total;
        }
        ...
        public byte[] ReadBytes(int index, int len)
        {
            ...
            Array.Copy(_buf, index, buffer, 0, len);
            readIndex += len;
            return buffer;
        }
