...
public class Logger
{
    ...
    private static string ContructFinalString(Channel logChannel, Priority priority, string message, bool shouldColour)
    {
        ...
        if (channelToColour.TryGetValue(logChannel, out var channelColour))
            return !shouldColour
                ? $"[{logChannel}] {message}"
                : $"<b><color={channelColour}>[{logChannel}] </color></b> <color={priortiyColour}>{message}</color>";
        channelColour = "black";
        Debug.LogErrorFormat("Please add colour for channel {0}", logChannel);

        return !shouldColour ? $"[{logChannel}] {message}" : $"<b><color={channelColour}>[{logChannel}] </color></b> <color={priortiyColour}>{message}</color>";
    }
