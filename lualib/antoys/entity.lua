...
function Entity:add(component)
    ...
    if self.components[name] then
        skynet.error("Entity: Trying to add Component '" .. name .. "', but it's already existing. Please use Entity:set to overwrite a component in an entity.")
    else
        self.components[name] = component
        if self.eventManager then
            self.eventManager:fireEvent(antoys.events.ComponentAdded(self, name))
        end
    end
end
...
function Entity:remove(name)
    if self.components[name] then
        ...
        skynet.error("Entity: Trying to remove non-existent component " .. name .. " from Entity. Please fix this")
    end
    if self.eventManager then
        self.eventManager:fireEvent(antoys.events.ComponentRemoved(self, name))
    end
end
