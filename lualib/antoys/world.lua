...
function World:initialize()
    ...
    self.rootEntity = antoys.Entity()
    self.singleRequirements = {}
    self.allRequirements = {}
    self.entityLists = {}
    self.eventManager = antoys.EventManager()
...
function World:addEntity(entity)
    -- Setting World eventManager as eventManager for entity
...
function World:removeEntity(entity, removeChildren, newParent)
    if self.entities[entity.id] then
        -- Removing the Entity from all Systems and World
        for _, component in pairs(entity.components) do
            local name = component.class.name
            if self.singleRequirements[name] then
                for _, system in pairs(self.singleRequirements[name]) do
        ...
        skynet.error("World: Trying to remove non existent entity from World.")
        if entity.id then
            skynet.error("World: Entity id: " .. entity.id)
        else
            skynet.error("World: Entity has not been added to any World yet. (No entity.id)")
        end
        skynet.error("World: Entity's components:")
        for index, component in pairs(entity.components) do
            skynet.error(index, component)
        end
    end
end

function World:addSystem(system, type)
    local name = system.class.name

    -- Check if the specified type is correct
    if type ~= nil and type ~= "draw" and type ~= "update" then
        skynet.error("World: Trying to add System " .. name .. "with invalid type " .. type .. ". Aborting")
        return
    end

    -- Check if a type should be specified
    if system.draw and system.update and not type then
        skynet.error("World: Trying to add System " .. name .. ", which has an update and a draw function, without specifying type. Aborting")
        return
    end
    ...
    if self.systemRegistry[name] and self.systemRegistry[name] ~= system then
        skynet.error("World: Trying to add two different instances of the same system. Aborting.")
        return
    end

    -- Adding System to World system reference table
    ...
    elseif not (system.update and system.draw) then
        if self.systemRegistry[name] then
            skynet.error("World: System " .. name .. " already exists. Aborting")
            return
        end
    end
...
function World:stopSystem(name)
    if self.systemRegistry[name] then
        ...
        skynet.error("World: Trying to stop not existing System: " .. name)
    end
end
...
function World:startSystem(name)
    if self.systemRegistry[name] then
        ...
        skynet.error("World: Trying to start not existing System: " .. name)
    end
end
...
function World:toggleSystem(name)
    if self.systemRegistry[name] then
        ...
        skynet.error("World: Trying to toggle not existing System: " .. name)
    end
end
...
function World:componentRemoved(event)
...
function World:componentAdded(event)
    local entity = event.entity
    local component = event.component
...
function World:checkRequirements(entity, system) -- luacheck: ignore self
