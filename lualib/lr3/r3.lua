...
if not ok then
    clear_tab = function (obj) obj = {} end
end

local ok, clone_tab = pcall(require, "table.clone")
if not ok then
    clone_tab =  function (object)
        local lookup_table = {}
...
local buf_len_prt = get_size_ptr()
DEBUG("buf_len_prt", DUMP(buf_len_prt))
...
local str_buff = get_string_buf(256)
DEBUG("str_buff", DUMP(str_buff))
...
local function insert_route(self, opts)
    ...
    local ret = r3.r3_route_set_attr(r3_node, host, remote_addr,
                                    remote_addr_bits)
    if ret == -1 then
        skynet.error("failed to set the attribute for route")
    end
...
-- method
----------------------------------------------------------------
for _, name in ipairs({"GET", "POST", "PUT", "DELETE", "PATCH", "HEAD", "OPTIONS"}) do
